...
void run_cplex(Timer& timer) {
    ...
    try{
        ...
        for (int i = 0; i < n_of_nodes; ++i) {
            ...
            IloExpr expr(env);
            // add the variable concerning the cur
            ...
            int ns = neighbors[i].size(), s; //= (neighbors.[i].size() == 1 ? 1 : 0.5*neighbors[i].size());
            if (ns % 2 != 0) s = 0.5*neighbors[i].size()+1;
            else s = 0.5*neighbors[i].size();
            cout <<"node " <<i <<": " <<s <<endl;
...
int main( int argc, char **argv ) {
   ...
   /* ifstream indata;
    ...
    cout <<"introduce the number of nodes in the graph" <<endl;
    cin >>n_of_nodes;
    neighbors.resize(n_of_nodes);
    cout <<"introduce the number of arcs in the graph" <<endl;
    cin >>n_of_arcs;
    
    cout <<"type in all pairs of neighbour nodes in the graph. Nodes start at 0" <<endl;
    int u, v;
    for (int i = 0; i < n_of_arcs; ++i){
        cout <<"edge " <<i <<":" <<endl;
        cin >>u >>v;
